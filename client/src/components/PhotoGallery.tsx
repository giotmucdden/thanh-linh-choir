...
const GALLERY_ROW_1: GalleryImageData[] = [
...
const GALLERY_ROW_2: GalleryImageData[] = [
...
const GalleryImage = memo(function GalleryImage({ src, alt }: GalleryImageProps) {
  return (
...
const GalleryRow = memo(function GalleryRow({
  ...
  const fadeGradient = "from-[oklch(0.18_0.06_240)] to-transparent";

  return (
    <div className="relative mb-4 last:mb-0">
      <div
        className={`flex gap-4 ${trackClass}`}
      ...
      {/* Edge fade gradients */}
      <div className={`absolute inset-y-0 left-0 w-24 bg-gradient-to-r ${fadeGradient} pointer-events-none z-10`} />
      <div className={`absolute inset-y-0 right-0 w-24 bg-gradient-to-l ${fadeGradient} pointer-events-none z-10`} />
    </div>
  );
});
...
const SectionHeader = memo(function SectionHeader({ subtitle, title }: SectionHeaderProps) {
  return (
...
function PhotoGallery() {
  ...
  return (
    <section
      id="gallery"
      className="py-20 bg-[oklch(0.18_0.06_240)] overflow-hidden"
    >
      <SectionHeader
        subtitle={t(lang, "gallery_subtitle")}
        title={t(lang, "gallery_title")}
      />

      <GalleryRow
        images={GALLERY_ROW_1}
        direction="left"
        keyPrefix="r1"
      />

      <GalleryRow
        images={GALLERY_ROW_2}
        direction="right"
        keyPrefix="r2"
      />
    </section>
